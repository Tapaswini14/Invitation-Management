...
@Component({
  selector: 'app-thread-ceremony',
  templateUrl: './thread-ceremony.component.html',
  styleUrls: ['./thread-ceremony.component.css'],
})
export class ThreadCeremonyComponent {
  selectedFile: File[] = [];
  submitted = false;
  ...
  secondFormGroup = this._formBuilder.group({
    ...
    nick_name: ['', [Validators.required, this.usernameValidator]],
  });
  ...
  fifthFormGroup = this._formBuilder.group({
    thread_date: ['', Validators.required],
    thread_time: ['', Validators.required],
    threadAddress: ['', Validators.required],
  });
  ...
  addThreadCeremonyCelebrant() {
    const newData = this.secondFormGroup.value;
    const invitationData = {
      celebrant_name: newData.CelebrantName,
      celebrant_nick_name: newData.nick_name,
      invitation_id: localStorage.getItem('invitation_id'),
    };

    this.tmApi.addThreadCeremonyCelebrant(invitationData).subscribe(
      (response: any) => {
        this.apiStatus = response.code;
        if (this.apiStatus === 200) {
          this.snackBar.open('Successfully Created Invitation', 'Close', {
  ...
  addThreadCeremonyEventDetails() {
    const newData = this.fifthFormGroup.value;
    let dateInForm = this.fifthFormGroup.value.thread_date;
    if (dateInForm) {
      ...
      const invitationData = {
        party_date: datetime,
        party_time: newData.thread_time,
        party_addr: newData.threadAddress,
        invitation_id: localStorage.getItem('invitation_id'),
      };

      this.tmApi.addThreadCeremonyEventDetails(invitationData).subscribe(
        (response: any) => {
          this.apiStatus = response.code;
          if (this.apiStatus === 200) {
  ...
  contactInvitation() {
    ...
    const invitationData = {
      ...
      mobile_no: newData.phone_number,
    };
