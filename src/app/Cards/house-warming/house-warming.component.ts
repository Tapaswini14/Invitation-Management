...
@Component({
  ...
  styleUrls: ['./house-warming.component.css'],
})
export class HouseWarmingComponent {
  ...
  secondFormGroup = this._formBuilder.group({
    ...
    partner_name: ['', [Validators.required, this.usernameValidator]],
  });
  thirdFormGroup = this._formBuilder.group({});
  fourthFormGroup = this._formBuilder.group({
    housewarming_date: ['', Validators.required],
    housewarming_time: ['', Validators.required],
    housewarming_address: ['', Validators.required],
  });
  ...
  createInvitation() {
    ...
    this.tmApi.createhouseWarmingInvitation(invitationData).subscribe(
      (response: any) => {
  ...
  addHouseWarmingDetails() {
    const newData = this.secondFormGroup.value;
    const invitationData = {
      house_warming_partner_name: newData.name,
      house_warming_name: newData.partner_name,
      invitation_id: localStorage.getItem('invitation_id'),
    };

    this.tmApi.addHouseWarmingDetails(invitationData).subscribe(
      (response: any) => {
        this.apiStatus = response.code;
        if (this.apiStatus === 200) {
  ...
  houseEventDetails() {
    ...
    let dateInForm = this.fourthFormGroup.value.housewarming_date;
    if (dateInForm) {
      ...
      const newData = {
        house_warming_date: datetime,
        house_warming_time: formData.housewarming_time,
        new_home_addr: formData.housewarming_address,
        invitation_id: localStorage.getItem('invitation_id'),
      };
      this.tmApi.addHouseWarmingEventDetails(newData).subscribe(
        (response: any) => {
          this.apiStatus = response.code;
          if (this.apiStatus === 200) {
